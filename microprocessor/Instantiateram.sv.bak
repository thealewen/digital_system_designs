...
module Instantiateram( input Reset,
	...
	always_comb begin
		case(address)
			...
			16'd22: data =    opLDR(R3, R0, inSW)      ;       // Load switches
			16'd23: data =    opPSE(12'h802)           ;       // Checkpoint 2 - prepare to input (lower)
			16'd24: data =    opLDR(R4, R0, inSW)      ;       // Load switches again
			16'd25: data =    opAND(R1, R3, R4)        ;       // Combine bytes; R1: A
			16'd26: data =    opPSE(12'h803)           ;       // Checkpoint 3 - prepare to input (upper)
			16'd27: data =    opLDR(R3, R0, inSW)      ;       // Load upper byte 2
			16'd28: data =    opPSE(12'h804)           ;       // Checkpoint 4 - prepare to input (lower)
			16'd29: data =    opLDR(R4, R0, inSW)      ;       // Load lower byte 2
			16'd30: data =    opAND(R2, R3, R4)        ;       // Combine bytes; R2: B
			16'd31: data =    opNOT(R3, R1)            ;       // R3: A'
			16'd32: data =    opAND(R3, R3, R2)        ;       // R3: A'B
			16'd33: data =    opNOT(R3, R3)            ;       // R3: (A'B)'
			16'd34: data =    opNOT(R4, R2)            ;       // R4: B'
			16'd35: data =    opAND(R4, R4, R1)        ;       // R4: AB'
			16'd36: data =    opNOT(R4, R4)            ;       // R4: (AB')'
			16'd37: data =    opAND(R3, R3, R4)        ;       // R3: (A'B)'(AB')'
			16'd38: data =    opNOT(R3, R3)            ;       // R3: ((A'B)'(AB')')'
			16'd39: data =    opSTR(R3, R0, outHEX)    ;       // Output
			16'd40: data =    opPSE(12'h405)           ;       // Checkpoint 5 - read output
			16'd41: data =    opBR(nzp, -21)           ;       // Repeat
